...
// Safe localStorage access — throws in private/incognito mode.
const menuStorage = {
  get(key) {
    try {
...
function initializeActiveMenuItem() {
  ...
  // into a CSS selector (which would break if it ever contained `"`, `]`, `\`).
  const activeMenuItem = menuStorage.get('activeMenuItem');
  if (activeMenuItem) {
    const activeLink = menuLinks.find(link => link.getAttribute('href') === activeMenuItem);
  ...
  navMenu.addEventListener('click', (event) => {
    ...
    menuStorage.set('activeMenuItem', link.getAttribute('href'));
  });
}
